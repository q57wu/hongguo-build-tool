...
def do_batch_assign(account_ids_text, dramas_text, ids_per_group, dramas_per_group,
    ...
    # 5. 分组 + 配对（剧循环复用：当 ID 组数 > 剧组数时，剧从头循环分配）
    ...
    if not id_groups or not drama_groups:
        return {"ok": False, "error": "分组后无法配对（ID组或剧组为空）"}

    # 以 ID 组数为准，剧组循环复用
    pair_count = len(id_groups)
    reuse_dramas = pair_count > len(drama_groups)
    ...
    for i in range(pair_count):
        ids = id_groups[i]
        dramas = drama_groups[i % len(drama_groups)]  # 循环取剧组
        content = "\n".join(ids) + "\n\n"
        drama_parts = []
        for drama in dramas:
        ...
        reuse_tag = "（复用）" if reuse_dramas and i >= len(drama_groups) else ""
        header = f"═══ 第 {i+1} 组{reuse_tag}（{len(ids)} 个ID，{len(dramas)} 部剧：{drama_names_str}）═══"
        all_contents.append(header + "\n" + content)

    result = ("\n" + "=" * 50 + "\n").join(all_contents)

    reuse_hint = f"，剧数据已复用到 {pair_count} 组" if reuse_dramas else ""
    return {
        "ok": True,
        "result": result,
        "summary": f"✅ 成功分配 {pair_count} 组（共 {len(all_ids)} 个ID，{len(all_dramas)} 部剧{reuse_hint}）"
    }
...
def do_material_push(drama_names, account_id, log_func, stop_event, platform="安卓"):
    ...
    from backend.selectors import load_selectors
    _sel = load_selectors().get("material_push_page", {})
    ...
    SEL_DIALOG = _sel.get("dialog", ".arco-drawer, .arco-modal")
    SEL_OPTION = _sel.get("select_option", ".arco-select-option")
    SEL_SEARCH_INPUT_ANDROID = _sel.get("search_input_android", "input[placeholder='请输入素材名称']")
    SEL_SEARCH_INPUT_IOS = _sel.get("search_input_ios", "#search_value_material_input")
    SEL_SEARCH_BTN = _sel.get("search_btn", "button.distribution_search:has-text('搜索')")
    SEL_TABLE_ROW = _sel.get("table_row", "tbody tr.arco-table-tr")
    ...
    def _close_blocking_dialogs(page):
        try:
            blocking = page.locator(
                _sel.get("blocking_overlay", ".arco-drawer-mask, .arco-modal-mask, .arco-drawer, .arco-modal"))
            if blocking.count() == 0:
                return
        except Exception:
            return
        ...
        for _ in range(3):
            try:
                ...
                pass
        close_selectors = [
            _sel.get("close_icon_drawer", ".arco-drawer .arco-drawer-close-icon"),
            _sel.get("close_icon_modal", ".arco-modal .arco-modal-close-icon"),
            _sel.get("close_btn_drawer", ".arco-drawer button[aria-label='Close']"),
            _sel.get("close_btn_modal", ".arco-modal button[aria-label='Close']"),
            _sel.get("close_icon_drawer2", ".arco-drawer .arco-icon-close"),
            _sel.get("close_icon_modal2", ".arco-modal .arco-icon-close"),
        ]
        for selector in close_selectors:
            try:
                ...
                continue
        try:
            page.locator(_sel.get("mask_overlay", ".arco-drawer-mask, .arco-modal-mask")).wait_for(
    ...
    def _push_all(page, name, account_id):
        # 切换分页为 100 条/页
        try:
            page_size_sel = page.locator(_sel.get("pagination_select", ".arco-pagination-option .arco-select-view"))
            if page_size_sel.count() > 0 and "100" not in page_size_sel.first.inner_text(timeout=3000):
                ...
                opt_100 = page.locator(_sel.get("pagination_option_100", ".arco-select-option:visible"), has_text="100 条/页")
                if opt_100.count() > 0:
                    opt_100.first.click()
        ...
        # 全选素材
        checkbox = page.locator(_sel.get("select_all_checkbox", "table thead .arco-checkbox-mask"))
...
def do_incentive_link_assign(params):
    ...
    material_ids_text = params.get("material_ids", "")
    ...
    material_ids = []
    if material_ids_text and material_ids_text.strip():
        material_ids = [mid.strip() for mid in re.split(r'[\s]+', material_ids_text.strip()) if mid.strip()]

    # 解析原始数据：每行 Tab 分隔，第一列含组标签，后面是链接
    raw_lines = [l.strip() for l in raw_data.strip().splitlines() if l.strip()]
    data_rows = []
    for line in raw_lines:
        ...
        if not links:
            continue
        # 从第一列提取"组X"作为标题
        group_match = re.search(r'组\d+', col0)
        title = group_match.group(0) if group_match else col0
        data_rows.append({"title": title, "links": links})

    if not data_rows:
        return {"ok": False, "error": "未识别到有效的原始数据行，请确认格式：Tab分隔"}

    # 解析账户ID列表（取每行第一个字段作为ID）
    account_lines = [l.strip() for l in account_ids.strip().splitlines() if l.strip()]
    if not account_lines:
        return {"ok": False, "error": "未识别到有效的账户ID"}
    ...
    # 每 N 个 ID 分一组
    id_groups = [parsed_ids[i:i+ids_per_group] for i in range(0, len(parsed_ids), ids_per_group)]

    # 组间分隔
    group_sep = '\n' * (spacing + 1)

    # 一一对应：第 N 行链接配第 N 组 ID
    total_blocks = min(len(data_rows), len(id_groups))
    output_blocks = []
    ...
    for i in range(total_blocks):
        ...
        # ID 列表
        id_lines = '\n'.join(group)
        # 链接行（每个链接末尾带 Tab，方便粘贴）
        link_lines = '\n'.join(l + '\t' for l in row["links"])
        # 素材ID（空格分隔，一行）
        material_line = ' '.join(material_ids) if material_ids else ''
    ...
    final_result = group_sep.join(output_blocks)

    warning = ''
    if len(data_rows) != len(id_groups):
        warning = f"（提示：{len(data_rows)} 行链接，{len(id_groups)} 个ID组，按较少的 {total_blocks} 组输出）"

    return {
        "ok": True,
        "result": final_result,
        "summary": f"✅ 共生成 {total_blocks} 组（{len(parsed_ids)} 个账户ID）{warning}"
    }
...
def do_crawl_material(drama_names, min_cost, min_count, log_func, stop_event):
    ...
    crawl_mod = None
    try:
        from backend.services import crawl_material as _crawl_mod
...
def parse_and_add_to_profile(profile_key, result_text):
    ...
    # 记录分配日志
    try:
        from backend.services.assign_log_service import add_assign_log
        add_assign_log(profile_key, "normal", groups, source="parse_and_add_to_profile")
    except Exception:
        pass  # 日志记录失败不影响主流程
...
def add_incentive_groups_to_profile(profile_key, result_text):
    ...
    输出格式（do_incentive_link_assign 产出）：
      组X
      ID1
      ID2
      ...
    ...
    组间用空行分隔，组内用 \\t\\t 行分隔 ID 与链接。
    解析逻辑：逐行扫描，遇到"组X"开头则开始新组。
    """
    from backend.config_manager import load_config, save_config
    ...
    # 逐行扫描，遇到"组X"标题行则开始新组
    raw_groups = []  # 每项 = {"label": str, "lines": [str]}
    current = None
    ...
    for line in result_text.splitlines():
        ...
        if not clean:
            continue
        # "组X" 标题行 → 开始新组
        if re.match(r'^组\d+', clean):
            current = {"label": clean, "lines": []}
            raw_groups.append(current)
        elif current is not None:
            current["lines"].append(clean)

    # 如果没有以"组X"开头的行，把全部内容当成一个组
    if not raw_groups:
        all_lines = [l.strip().rstrip('\t') for l in result_text.splitlines() if l.strip().rstrip('\t')]
        if all_lines:
            raw_groups = [{"label": "组1", "lines": all_lines}]
    ...
    for rg in raw_groups:
        ...
        for line in rg["lines"]:
            ...
            # 空格分隔的多个长数字 → 素材ID
            elif re.match(r'^\d[\d\s]+\d$', line) and len(line.split()) > 1:
                material_ids = [x.strip() for x in line.split() if x.strip()]
            # 其他行忽略（如 \t\t 分隔符等）

        if not account_ids:
            continue

        # 根据链接内容自动识别类型（点击/展示/播放）
        click_url = ""
        show_url = ""
        play_url = ""
        ...
        # 如果有素材ID，存入组配置
        if material_ids:
            group["material_ids"] = material_ids
        groups.append(group)
    ...
    try:
        ...
        for g in groups:
            log_groups.append({
                ...
                "material_ids": g.get("material_ids", []),
            })
        add_assign_log(profile_key, "incentive", log_groups, source="add_incentive_groups_to_profile")
    except Exception:
        pass  # 日志记录失败不影响主流程
